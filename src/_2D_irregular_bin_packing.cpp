...
ItemState dnc_approach::translateItemState(ItemState &itemState, Point point)
{
    std::vector<Polygon> polygons = itemState.first;
    for (Polygon &polygon : polygons)
    {
        polygon = boost_geo_util::translatePolygon(polygon, point);
    }
    Polygon polygon = boost_geo_util::translatePolygon(itemState.second, point);
    return {polygons, polygon};
}

/**
 * rotates the ItemState with respect to ref by angle degree CW
*/
ItemState dnc_approach::rotateItemState(ItemState &itemState, Point ref, double angle)
{
    std::vector<Polygon> polygons = itemState.first;
    for (Polygon &polygon_i : polygons)
    {
        polygon_i = boost_geo_util::rotatePolygonCW_D(polygon_i, angle, ref);
    }
    Polygon polygon = boost_geo_util::rotatePolygonCW_D(itemState.second, angle, ref);
    return {polygons, polygon};
}
...
*/
ItemState dnc_approach::mergeHeuristics1(ItemState &itemStateA, ItemState &itemStateB)
{
    Polygon polygonA = itemStateA.second;
    Polygon polygonB = itemStateB.second;

    std::vector<Point> polygon_a = polygonA.outer();
    int n = polygon_a.size();
    
}

// ItemState dnc_approach::Normalize(ItemState A)
// {
...
// bool dnc_approach::doesOverlap(ItemState A, ItemState B)
// {
//     double area = 0;
...
// ItemState dnc_approach::merge(ItemState A, ItemState B)
// {
//     if (get<1>(A).size() == 0)
//     {
//         return B;
...
/**
 * merges two ItemStates after dividing
*/
ItemState dnc_approach::mergeItemStates(ItemState &itemStateA, ItemState &itemStateB)
{
    ItemState bestItemState;
    if (boost_geo::area(itemStateA.second) < boost_geo::area(itemStateB.second))
    {
        std::swap(itemStateA, itemStateB);
    }
    double minAreaOfMBR = INF; // MBR = Minimum Bound Rectangle of a ploygon

    for (int i = 0; i < 360; i += 90)
    {
        Point rotationReferece(0, 0);
        double rotationAngle = i;
        ItemState rotatedItemStateA = rotateItemState(itemStateA, rotationReferece, rotationAngle);
        // ItemState rotatedItemStateLeft = Rotate(itemStateLeft, Point(0, 0), i);
        // rotatedItemStateLeft = Normalize(rotatedItemStateLeft);
        // for (int j = 0; j < 360; j += 90)
        // {
        //     ItemState D = Rotate(itemStateRight, {0, 0}, j);
        //     D = Normalize(D);
        //     ItemState E = merge(rotatedItemStateLeft, D);
 ...
 * divide and conquer
*/
ItemState dnc_approach::split(std::vector<Polygon> &polygons, int left, int right)
{
    if (left == right)
    {
        return {{polygons[left]}, polygons[left]};
    }
    int mid = left + (right - left) / 2;

    ItemState itemStateLeft = split(polygons, left, mid);
    ItemState itemStateRight = split(polygons, mid + 1, right);

    return mergeItemStates(itemStateLeft, itemStateRight);
}

ItemState dnc_approach::solution(std::vector<PolygonInput> &items)
{
