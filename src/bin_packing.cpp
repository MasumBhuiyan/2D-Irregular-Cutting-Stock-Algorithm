...
MultiPolygon cluster_util::generateInitialSolution(vector<Polygon> &inputPolygons, long double width, string outputLocation)
{
	std::cout << "cluster values calculating...................: " << std::endl;
	vector<vector<vector<vector<long double>>>> clusterValues = cluster_util::getClusterValues(inputPolygons, outputLocation + "/all_pair_nfp_wkt_files");

	std::cout << "calling getPerfectClustering()...............: " << std::endl;
	...
	string clusterStorageDirectory = outputLocation + "/initial_solution/clusters";
	mkdir(clusterStorageDirectory.c_str(), 0777);
	...
	for (auto &clusterId : clusterPairs)
	{
		...
		if (boost_geo::is_convex(nfp.outer()) == false)
		{
			...
			geo_util::visualizeCluster(polygon_i, {polygon_j}, nfp, clusterStorageDirectory, testId++);
		}
	}
	...
	for (auto cluster : clusters)
	{
		geo_util::visualize(cluster, clusterStorageDirectory, "sample");
		std::cout << "Final Cluster: " << boost_geo::wkt(cluster) << "\n";
	}
	...
	std::cout << "generateInitialSolution() done." << std::endl;
	...
	vector<Polygon> &polygons, long double width, string outputLocation, string datasetName, long double runTimeDuration)
{
	...
	MultiPolygon initialPacking = cluster_util::generateInitialSolution(polygons, width, outputDirectoryName);
